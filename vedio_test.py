    ...
    def test(self):
        ...
        while(cap.isOpened()):
            ...
            if(self.selectingObject):
                ...
                #===================update-net======================
                frame,box_ori,img_p,box_p,offset,ratio=self.reader.get_vedio_data(img=frame,frame_n=1,pre_box=pre_box)
                img_p=np.expand_dims(img_p,axis=0)
                feed_dict={img_t:img_p,conv_c:conv_c_,conv_r:conv_r_}
                t0 = time.time()
                ...
                pre_box=self.recover(frame,bbox,offset,ratio,pre_box)#[x1,y1,x2,y2]
